...
methods='\n'.join(block(s,key) for key in ['void instance_ahnkahet::OnCreatureDespawn(', 'void instance_ahnkahet::FinishInsanityVisage(', 'void instance_ahnkahet::HandleInsanityClear()', 'void instance_ahnkahet::HandleInsanitySwitch('])
...
struct Creature{unsigned guid,owner=0,despawns=0,deathCasts=0;bool temp=true;instance_ahnkahet*instance;std::set<unsigned>auras;unsigned GetObjectGuid(){return guid;}bool IsTemporarySummon(){return temp;}unsigned GetSpawnerGuid(){return owner;}void CastSpell(Creature*,unsigned id,unsigned){if(id==57555)++deathCasts;}void RemoveAurasDueToSpell(unsigned id){auras.erase(id);}void ForcedDespawn();};
...
struct instance_ahnkahet{
 ...
 void OnCreatureDespawn(Creature*);void FinishInsanityVisage(Creature*,bool);void HandleInsanityClear();void HandleInsanitySwitch(Player*);
};
__HELPER__
__METHODS__
...
int main(){
 ...
 Map map;Player first{11},second{12},dead{13};dead.alive=false;map.players={{11,&first},{12,&second},{13,&dead}};instance_ahnkahet instance{&map};Creature boss{1,0,0,0,false,&instance},one{21,11,0,0,true,&instance},two{22,12,0,0,true,&instance};instance.boss=&boss;map.creatures={{21,&one},{22,&two}};
 ...
 std::cout<<"PASS: duplicate/reentrant visage death, despawn, wipe/completion cleanup, attempt timer, unrelated phases, failed phase transfer and dead destination exclusion\n";
}

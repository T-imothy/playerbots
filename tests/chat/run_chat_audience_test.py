...
p=argparse.ArgumentParser();p.add_argument('--generate-only',action='store_true');p.add_argument('--output',type=Path,required=True);a=p.parse_args()
out=a.output.resolve();out.mkdir(parents=True,exist_ok=True)
s=(r/'playerbot/strategy/actions/SayAction.cpp').read_text(encoding='utf-8')
start=s.index('void ChatReplyAction::ChatReplyDo(')
end=s.index('\nbool ChatReplyAction::HandleThunderfuryReply',start)
s=s[start:end]
elig=s[s.index('    if (bot->GetPlayerbotAI() &&'):s.index('    {\n        Player* player =',s.index('    if (bot->GetPlayerbotAI() &&'))]
guard=re.search(r'        if \(!chanName.empty\(\) && !ai->ChannelHasRealPlayer\(chanName\)\)\s+return;',s).group()
old=(r/'tests/chat/ChatAudienceFallbackReference.inc').read_text(encoding='utf-8')
...
struct AI;struct Player{AI* GetPlayerbotAI();};
...
bool hasAI=true;
AI* Player::GetPlayerbotAI(){return hasAI?&instance:nullptr;}

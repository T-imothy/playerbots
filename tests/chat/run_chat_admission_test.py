...
p=argparse.ArgumentParser();p.add_argument('--generate-only',action='store_true');p.add_argument('--output',type=Path,required=True);args=p.parse_args()
out=args.output.resolve();out.mkdir(parents=True,exist_ok=True)
before=(r/'tests/chat/ChatAdmissionReference.inc').read_text(encoding='utf-8')
after=(r/'playerbot/PlayerbotAI.cpp').read_text(encoding='utf-8')
...
struct Player {uint32 guid=2;bool bg=false;uint32 GetObjectGuid(){return guid;}const char* GetName(){return "Recipient";}bool InBattleGround(){return bg;}bool isRealPlayer(){return senderReal;}};
...
if not args.generate_only:
    subprocess.run(['cl','/nologo','/std:c++20','/EHsc','/O2','/UNDEBUG','/I'+str(r),'test.cpp','/Fe:test.exe'],cwd=out,check=True)

...
struct Unit{ObjectGuid guid;float x=0,y=0,z=0,health=100;uint32 map=489;bool alive=true,world=true,combat=false,teleport=false;
...
struct Facade{bool friendly=false;bool IsFriendlyTo(Unit*,Unit*){return friendly;}bool isSpawned(GameObject* g){return g->spawned;}}sServerFacade;
...
struct AttackEnemyFlagCarrierAction{Player* bot;AiObjectContext* context;bool isUseful();};
...
'''
    ...
    methods='\n'.join(block(values,m) for m in ('BattleGroundTypeId ai::ActualBattlegroundType','bool ai::IsBattlegroundFlagCarrier',
        'WarsongObjective WarsongObjectiveValue::Calculate','bool ai::ShouldAdvanceWarsongObjective'))
...
int main(){
 ...
 for(Team team:{ALLIANCE,HORDE}){
  ...
  AttackEnemyFlagCarrierAction attack{&bot,context};assert(attack.isUseful());
  ...
  enemyFC.x=bot.x+10;enemyFC.combat=true;enemyFC.selection=bot.guid;

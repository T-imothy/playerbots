...
def clean(path):return '\n'.join(s for s in path.read_text(encoding="utf-8").splitlines() if not s.startswith(('#include','#pragma')))
...
struct Player;
struct GameObjectInfo{float GetInteractionDistance(){return 5;}struct {unsigned reqParticipants=3,animSpell=11206,spellId=11568;}summoningRitual;};
struct GameObject{bool world=true,spawned=true,same=true;unsigned guid=9,entry=130511,owner=0,users=1;int loot=0,type=18;float x=0;GameObjectInfo info;
 ...
 unsigned GetObjectGuid(){return guid;}unsigned GetUniqueUseCount(){return users;}bool IsAtInteractDistance(Player*);};
...
bool GameObject::IsAtInteractDistance(Player*p){return p->GetDistance(this)<=info.GetInteractionDistance();}

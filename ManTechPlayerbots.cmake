# Current ManTech CMaNGOS Classic baseline on the native Penqle host.
set(PB_ROOT "${CMAKE_CURRENT_LIST_DIR}")
if(TORTOISE_MODULE_CMAKE_PHASE STREQUAL "DISCOVERY")
  if(NOT BUILD_PLAYERBOTS)
    message(FATAL_ERROR "ManTechPlayerbots requires BUILD_PLAYERBOTS=ON for host lifecycle implementations")
  endif()
  file(GLOB_RECURSE PB_SOURCES CONFIGURE_DEPENDS
    "${PB_ROOT}/playerbot/*.cpp" "${PB_ROOT}/cmangos-ahbot/*.cpp")
  # Match the donor's Classic source selection; retain expansion sources in Git.
  list(FILTER PB_SOURCES EXCLUDE REGEX "/strategy/deathknight/")
  foreach(PB_SOURCE IN LISTS PB_SOURCES)
    TW_ADD_SCRIPT("${PB_SOURCE}")
  endforeach()
  message(STATUS "ManTechPlayerbots: current ManTech Classic baseline c271085c92c380142a90a5514cf54a69b8f8ac37 with native host adapters")
  return()
endif()
if(NOT TORTOISE_MODULE_CMAKE_PHASE STREQUAL "POST_TARGETS")
  return()
endif()
...
foreach(PB_TARGET ${TORTOISE_CURRENT_MODULE_TARGET})
  ...
  target_include_directories(${PB_TARGET} PUBLIC
    ...
    ${PB_ROOT}/src
    ${PB_ROOT}/playerbot
    ${PB_ROOT}/cmangos-ahbot
    ${PB_ROOT}/cmangos-compat-stubs
...
if(${CMAKE_PROJECT_NAME} MATCHES "TBC")
  configure_file(${PB_ROOT}/playerbot/aiplayerbot.conf.dist.in.tbc
                 ${CMAKE_BINARY_DIR}/aiplayerbot.conf.dist)
elseif(${CMAKE_PROJECT_NAME} MATCHES "WoTLK")
  configure_file(${PB_ROOT}/playerbot/aiplayerbot.conf.dist.in.wotlk
                 ${CMAKE_BINARY_DIR}/aiplayerbot.conf.dist)
else()
  configure_file(${PB_ROOT}/playerbot/aiplayerbot.conf.dist.in
                 ${CMAKE_BINARY_DIR}/aiplayerbot.conf.dist)
endif()
...
configure_file(${PB_ROOT}/cmangos-ahbot/ahbot.conf.dist.in
               ${CMAKE_BINARY_DIR}/ahbot.conf.dist @ONLY)
install(FILES ${CMAKE_BINARY_DIR}/ahbot.conf.dist DESTINATION ${CONF_INSTALL_DIR})

# The ManTech LLM client retains its HTTP/TLS implementation.
get_filename_component(PB_OPENSSL_PARENT "${OPENSSL_INCLUDE_DIR}" DIRECTORY)
target_include_directories(${TORTOISE_CURRENT_MODULE_TARGET} PRIVATE
  "${OPENSSL_INCLUDE_DIR}" "${PB_OPENSSL_PARENT}")

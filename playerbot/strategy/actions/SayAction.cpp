...
bool SayAction::Execute(Event& event)
{
    ...
    if (bot->IsInWorld())
    {
        if (AreaTableEntry const* area = GetAreaEntryByAreaID(sServerFacade.GetAreaId(bot)))
            placeholders["<subzone>"] = area->area_name;
    }
...
futurePackets ChatReplyAction::GenerateResponsePacketsAsync(const std::string json,
    const WorldPacket chatTemplate, const WorldPacket emoteTemplate, const WorldPacket systemTemplate,
    const std::string startPattern, const std::string endPattern, const std::string deletePattern,
    const std::string splitPattern, bool debug)
{
    // Value copies only; no bot, AI, session, context or map enters the worker.
    return SubmitPlayerbotChatGeneration([=]() {
        return GenerateResponsePackets(json, chatTemplate, emoteTemplate, systemTemplate,
            startPattern, endPattern, deletePattern, splitPattern, debug);
    });
}
...
void ChatReplyAction::ChatReplyDo(Player* bot, uint32 type, uint32 guid1, uint32 guid2, std::string msg, std::string chanName, std::string name)
{
        ...
        )
    {
        ...
        // No audience for this LLM-managed channel: finish this reply here.
        // Clearing the sender instead fell through into canned replies, so
        // bots recursively answered one another only after the humans left.
        // Special replies above and non-LLM/blocked channels keep their paths.
        ...
        if (player)
        {
            ...
            if (player != bot && (IsRealPlayer(player) || (sPlayerbotAIConfig.llmBotToBotChatChance && urand(0, 99) < sPlayerbotAIConfig.llmBotToBotChatChance)))
            {
                ...
                futurePackets futPackets = ChatReplyAction::GenerateResponsePacketsAsync( json, chatTemplate, emoteTemplate, systemTemplate, startPattern, endPattern, deletePattern, splitPattern, debug);

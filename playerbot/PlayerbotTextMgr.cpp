...
void PlayerbotTextMgr::LoadBotTexts()
{
    ...
    auto results = std::unique_ptr<QueryResult>(WorldDatabase.PQuery("SELECT `name`, `text`, `say_type`, `reply_type`, `text_loc1`, `text_loc2`, `text_loc3`, `text_loc4`, `text_loc5`, `text_loc6`, `text_loc7`, `text_loc8` FROM `ai_playerbot_texts`"));
    int count = 0;
    if (results)
    {
        do
...
void PlayerbotTextMgr::LoadBotTextChance()
{
    ...
    {
        auto results = std::unique_ptr<QueryResult>(WorldDatabase.PQuery("SELECT name, probability FROM ai_playerbot_texts_chance"));
        if (results)
        {
            do
            {

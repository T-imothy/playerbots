...
void BotLog::Initialize(const char* logFile, const char* logsDir, bool debugEnabled)
{
    if (!logFile || logFile[0] == '\0')
        return;  // empty â†’ fall through to Log::Instance() on every call
...
{
    if (!m_debugEnabled && !Log::Instance().HasLogLevelOrHigher(LOG_LVL_DETAIL))
        return;
...
void BotLog::outPerformance(const char* fmt, ...)
{
    BOTLOG_IMPL("[PERFORMANCE] ", outBasic)
}

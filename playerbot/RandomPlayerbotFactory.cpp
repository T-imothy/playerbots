...
void RandomPlayerbotFactory::EnsureNamesInitialized()
{
    ...
    auto result = std::unique_ptr<QueryResult>(CharacterDatabase.PQuery("SELECT n.gender, n.name, e.guid FROM ai_playerbot_names n LEFT OUTER JOIN characters e ON e.name = n.name"));
    if (!result)
    {
    ...
    if (allNames.count(NameRaceAndGender::DwarfMale) == 0)
    {
        ...
        auto oldResult = std::unique_ptr<QueryResult>(CharacterDatabase.PQuery("SELECT e.name FROM characters e"));
        if (oldResult)
        {
            do
            {
...
void RandomPlayerbotFactory::CreateRandomBots()
{
    ...
    auto values = std::unique_ptr<QueryResult>(CharacterDatabase.Query(
    ...
    if (sPlayerbotAIConfig.deleteRandomBotAccounts || delAccs)
    {
        ...
        for (uint32 accountNumber = 0; accountNumber < maxAccountNum; ++accountNumber)
        {
            ...
            auto result = std::unique_ptr<QueryResult>(LoginDatabase.PQuery("SELECT id FROM account where username = '%s'", accountName.c_str()));
            if (!result)
                continue;
        ...
        if (!delFriends)
        {
            auto result = std::unique_ptr<QueryResult>(CharacterDatabase.PQuery("SELECT friend FROM character_social WHERE flags='%u'", SOCIAL_FLAG_FRIEND));
            if (result)
            {
                do
                {
        ...
        auto results = std::unique_ptr<QueryResult>(LoginDatabase.PQuery("SELECT id FROM account where username like '%s%%'", sPlayerbotAIConfig.randomBotAccountPrefix.c_str()));
        if (results)
        {
            ...
            do
            {
                ...
                if (!delFriends)
                {
                    ...
                    auto result = std::unique_ptr<QueryResult>(CharacterDatabase.PQuery("SELECT guid FROM characters WHERE account='%u'", accId));
                    if (result)
                    {
                        do
                        {
    ...
    if (!sPlayerbotAIConfig.randomBotAutoCreate)
    {
        ...
        auto accounts = std::unique_ptr<QueryResult>(LoginDatabase.PQuery("SELECT id FROM account WHERE username LIKE '%s%%' ORDER BY id", prefix.c_str()));
        if (accounts)
            do
            {
    ...
    auto temporarybots = std::unique_ptr<QueryResult>(CharacterDatabase.Query("SELECT characters.guid, characters.account FROM ai_playerbot_random_bots JOIN characters ON (characters.guid = ai_playerbot_random_bots.bot AND characters.name = ai_playerbot_random_bots.data) WHERE ai_playerbot_random_bots.event = 'temporary'"));
    ...
    auto temporaryAccounts = std::unique_ptr<QueryResult>(LoginDatabase.PQuery("SELECT id FROM account WHERE username like '%s%%' and id >= %u", sPlayerbotAIConfig.randomBotAccountPrefix.c_str(), sPlayerbotAIConfig.randomBotAccountCount));
    ...
    auto poolCount = std::unique_ptr<QueryResult>(LoginDatabase.PQuery("SELECT COUNT(*) FROM account WHERE username LIKE '%s%%'", poolPrefix.c_str()));
    if (!poolCount)
    {
    ...
    for (uint32 accountNumber = 0; existingAccounts + scheduledAccounts < sPlayerbotAIConfig.randomBotAccountCount; ++accountNumber)
    {
        std::ostringstream out; out << sPlayerbotAIConfig.randomBotAccountPrefix << accountNumber;
        std::string accountName = out.str();
        auto results = std::unique_ptr<QueryResult>(LoginDatabase.PQuery("SELECT id FROM account where username = '%s'", accountName.c_str()));
        if (results)
        {
            continue;
        }
    ...
    auto countResult = std::unique_ptr<QueryResult>(CharacterDatabase.Query("SELECT COUNT(guid) FROM characters"));
    if (countResult)
    ...
    auto poolResult = std::unique_ptr<QueryResult>(LoginDatabase.PQuery("SELECT id FROM account WHERE username LIKE '%s%%' ORDER BY id", poolPrefix.c_str()));
    if (poolResult)
...
void RandomPlayerbotFactory::CreateRandomGuilds()
{
    ...
    auto charAccounts = std::unique_ptr<QueryResult>(CharacterDatabase.PQuery(
...
{
    auto result = std::unique_ptr<QueryResult>(CharacterDatabase.Query("SELECT MAX(name_id) FROM ai_playerbot_guild_names"));
    if (!result)
    {
    ...
    result = std::unique_ptr<QueryResult>(CharacterDatabase.PQuery("SELECT n.name FROM ai_playerbot_guild_names n "
...
void RandomPlayerbotFactory::CreateRandomArenaTeams()
{
    ...
    for (; arenaTeamNumber < sPlayerbotAIConfig.randomBotArenaTeamCount; ++arenaTeamNumber)
    {
        ...
        auto results = std::unique_ptr<QueryResult>(CharacterDatabase.PQuery("SELECT `type` FROM ai_playerbot_arena_team_names WHERE name = '%s'", arenaTeamName.c_str()));
        if (!results)
        {
...
{
    auto result = std::unique_ptr<QueryResult>(CharacterDatabase.Query("SELECT MAX(name_id) FROM ai_playerbot_arena_team_names"));
    if (!result)
    {
    ...
    uint32 id = urand(0, maxId);
    result = std::unique_ptr<QueryResult>(CharacterDatabase.PQuery("SELECT n.name FROM ai_playerbot_arena_team_names n "

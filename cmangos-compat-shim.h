...
// Same native 0x4 control-loss flag, named DISABLE_MOVE in Turtle. Native
// scripted player waypoints set it and taxi admission checks it.
#ifndef UNIT_FLAG_CLIENT_CONTROL_LOST
#define UNIT_FLAG_CLIENT_CONTROL_LOST UNIT_FLAG_DISABLE_MOVE
#endif
...
// Native stores own all loaded custom-content entries. Consumers should iterate
// those maps directly; this helper retains the exclusive upper bound API for
// callers which only need the bound. It does not retain stale reload snapshots.
template <typename Store> uint32 NativeStoreEnd(Store const& store)
{
    uint32 highest = 0;
    for (auto const& entry : store) highest = std::max(highest, uint32(entry.first));
    return store.empty() ? 0 : highest + 1;
}
struct CmangosItemStorageProxy
{
    ...
    uint32 GetMaxEntry() const { return NativeStoreEnd(sObjectMgr.GetItemPrototypeMap()); }
};
...
struct CmangosFactionTemplateStoreProxy
{
    ...
    uint32 GetNumRows() const { return NativeStoreEnd(sObjectMgr.GetFactionTemplateMap()); }
};
...
struct CmangosFactionStoreProxy
{
    ...
    uint32 GetNumRows() const { return NativeStoreEnd(sObjectMgr.GetFactionMap()); }
};
...
struct CmangosCreatureStorageProxy
{
    ...
    uint32 GetMaxEntry() const { return NativeStoreEnd(sObjectMgr.GetCreatureInfoMap()); }
};
...
// Match native HandlePetSpellAutocastOpcode/Pet::ToggleAutocast. The caller
// must additionally own the pet and verify that it has learned this spell.
...
#define NAV_AREA_WATER AREA_WATER
#endif
#ifndef NAV_AREA_GROUND
#define NAV_AREA_GROUND AREA_GROUND
#endif
#ifndef NAV_AREA_GROUND_STEEP
#define NAV_AREA_GROUND_STEEP AREA_STEEP_SLOPE
#endif
#ifndef NAV_MAGMA_SLIME
#define NAV_MAGMA_SLIME (NAV_MAGMA | NAV_SLIME)
#endif
#ifndef NAV_GROUND_STEEP
#define NAV_GROUND_STEEP NAV_STEEP_SLOPES
#endif
...
struct CmangosAreaTriggerStoreProxy
{
    ...
    uint32 GetNumRows() const { return NativeStoreEnd(sObjectMgr.GetAreaTriggersMap()); }
};
...
#define ITEM_FLAG_UNIQUE_EQUIPPABLE ITEM_FLAG_UNIQUE_EQUIPPED
#endif
...
// Native result identities for baseline cast commands and combat diagnostics.
inline char const* GetSpellCastResultString(SpellCastResult result)
{
    switch (result)
    {
        ...
        default: return "UNKNOWN_SPELL_CAST_RESULT";
    }
}
...
{
    return IsSpellHaveEffect(spellInfo, SPELL_EFFECT_PERSISTENT_AREA_AURA);
}
...
struct CmangosGOStorageProxy
{
    ...
    uint32 GetMaxEntry() const { return NativeStoreEnd(sObjectMgr.GetGameObjectInfoMap()); }
};
...
// Optional module-owned voice data from the installed Turtle client DBC.
struct EmotesTextSoundEntry { uint32 Id, EmotesTextId, RaceId, SexId, SoundId; };
...
// Match the native explicit target selectors; area recipients are selected by
// Spell::FillTargetMap, not by the AI's single-target capability query.
...
// Native spell metadata queries used by ManTech's combat policies.
using Spells::GetSpellRadius;
inline bool IsPassiveSpell(SpellEntry const* info) { return info && info->IsPassiveSpell(); }
inline DiminishingGroup GetDiminishingReturnsGroupForSpell(SpellEntry const* info, bool triggered)
{ return info->GetDiminishingReturnsGroup(triggered); }
inline SpellCastResult GetErrorAtShapeshiftedCast(SpellEntry const* info, uint32 form)
{ return info->GetErrorAtShapeshiftedCast(form); }

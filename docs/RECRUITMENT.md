...
- Convenience `summon` requests are bounded and preserve native teleport checks. Combat, taxi/transport, charm and active transfers wait for safety rather than force an escape. Acceptance and arrival are separate statuses. Arrival requires completed transition, matching map AND instance, alive, out of combat, within 10 yards and line of sight. Same-map/different-instance shortcuts are refused. Native summon responses, warlock rituals, meeting-stone ritual interaction and recent teleport acknowledgement safeguards are preserved.
